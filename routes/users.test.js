...
describe("GET /users/:username", function () {
	...
	test("unauth for mismatched users", async function () {
		const resp = await request(app)
			.get(`/users/different_user`)
			.set("authorization", `Bearer ${u1Token}`);
		expect(resp.statusCode).toEqual(401);
	});
...
describe("DELETE /users/:username", function () {
	...
	test("works for user that matches :username", async function () {
		const resp = await request(app)
			.delete(`/users/u1`)
			.set("authorization", `Bearer ${u1Token}`);
		expect(resp.body).toEqual({ deleted: "u1" });
...
describe("POST /users/:username/jobs/:jobId", function () {
	...
	test("unauth for mismatched user", async function () {
		const resp = await request(app)
