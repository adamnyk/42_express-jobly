...
describe("GET /companies", function () {
	...
	test("filters companies by name.", async () => {
		...
		const resp2 = await request(app).get("/companies?name=3");
		expect(resp2.body).toEqual({
			companies: [comp3],
		});
	});

	test("filters companies by minimum number of employees.", async () => {
	...
	test("filters companies by maximum number of employees.", async () => {
	...
	test("multiple filters should work.", async () => {
		...
		const resp2 = await request(app).get("/companies?name=z&minEmployees=1&maxEmployees=2");
		expect(resp2.body).toEqual({
			companies: [],
		});
  });

  test("conflicting employee filters should throw an error", async () => {
		const resp = await request(app).get("/companies?maxEmployees=1&minEmployees=3");
		expect(resp.statusCode).toEqual(400);
  });
  
  test("unknown filters should throw an error", async () => {
		const resp = await request(app).get("/companies?weirdFilter=9");
		expect(resp.statusCode).toEqual(400);
  });
